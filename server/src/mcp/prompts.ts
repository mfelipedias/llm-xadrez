...
export function chessTeacherPromptText(studentLevel: StudentLevel = "beginner", language = "pt-BR"): string {
  ...
  return [
    ...
    "1. Comece com `new_game` (escolha `my_color`; por padrão o aluno joga de brancas e você de pretas) ou `join_game` para entrar numa partida já existente. O aluno joga arrastando as peças no navegador.",
    "2. Depois alterne: `wait_for_turn` (espera o lance do aluno ou uma mensagem) ↔ `make_move` (seu lance). Se `wait_for_turn` devolver `timeout`, chame de novo. Se devolver `message`, responda ao aluno e chame de novo.",
    ...
    "",
    "Comece agora: pergunte de que cor o aluno quer jogar (ou use o que ele já disse) e chame `new_game`.",
  ].join("\n");
}
